...
public class BrandCategoryMapperTests {
    ...
    public void testSelect(){
        Long id = 3L;
        BrandCategoryDetailVO brandCategoryDetailVO = mapper.getById(id);
        log.info("根据id{}查询完成: {}",id,brandCategoryDetailVO);
    }

    @Test
    public void testList() {
        List<BrandCategoryListItemVO> list = mapper.list();
        log.info("查询列表完成，结果集中的数据的数量{}" ,list.size());
        for (BrandCategoryListItemVO item : list) {
            log.info("{}",item);
        }
    }
}
